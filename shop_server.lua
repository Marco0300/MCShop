...
local function onlinePlayers() local list={}; local ok,entities=pcall(commands.getEntities,"@a"); if ok and type(entities)=="table" then for _,e in ipairs(entities) do local n=e.name or e.displayName; if n then list[n]=true end end end; return list end
...
local function handle(id,r)
 ...
 if r.action=="transfer" then if not validName(r.target) then return {success=false,error="Invalid recipient"} end; local amount=math.floor(tonumber(r.amount) or 0); if amount<1 then return {success=false,error="Invalid amount"} end; if a.balance<amount then return {success=false,error="Insufficient funds"} end; local target=account(r.target); a.balance=a.balance-amount; target.balance=target.balance+amount; log(s.username,"transfer_sent",r.target,1,amount); log(r.target,"transfer_received",s.username,1,amount); save(); return {success=true,balance=a.balance} end
