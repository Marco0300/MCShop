...
local adminKey="change-me"; local importURL=""
...
if fs.exists("shop_import_url") then local f=fs.open("shop_import_url","r"); importURL=f.readAll():gsub("%s+$",""); f.close() end
local catalog={
...
local function session(sid) local s=db.sessions[sid]; if s and s.expires>os.epoch("utc") then return s end; db.sessions[sid]=nil end
...
local function handle(id,r)
 ...
 local sid=r.clientKey and ("web:"..tostring(r.clientKey)) or id
 ...
 if r.action=="verify_pin" then
  ...
  db.pins[r.username]=nil; local a=account(r.username); db.sessions[sid]={username=r.username,expires=os.epoch("utc")+900000}; save(); return {success=true,username=r.username,balance=a.balance}
 end
 if r.action=="admin_login" then if tostring(r.key or "")~=adminKey then return {success=false,error="Invalid admin key"} end; adminSessions[sid]=true; return {success=true} end
 if r.action=="admin_logout" then adminSessions[sid]=nil; return {success=true} end
 if r.action=="admin_accounts" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; local out={}; for n,a in pairs(db.accounts) do table.insert(out,{username=n,balance=a.balance or 0}) end; table.sort(out,function(a,b) return a.username<b.username end); return {success=true,accounts=out} end
 if r.action=="admin_loans" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; local out={}; for n,l in pairs(db.loans) do table.insert(out,{username=n,principal=l.principal,remaining=l.remaining,arrears=l.arrears or 0,status=l.status,paidDays=l.paidDays}) end; table.sort(out,function(a,b) return a.username<b.username end); return {success=true,loans=out} end
 if r.action=="admin_loan_decide" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; local l=db.loans[r.username]; if not l or l.status~="pending" then return {success=false,error="No pending loan for that player"} end; if r.decision=="approve" then l.status="active"; l.lastPaidDay=os.day(); local a=account(r.username); a.balance=a.balance+l.netPayout; log(r.username,"loan_approved","loan",1,l.netPayout); save(); return {success=true,status="active"} elseif r.decision=="reject" then l.status="rejected"; save(); return {success=true,status="rejected"} end; return {success=false,error="Decision must be approve or reject"} end
 if r.action=="admin_balance" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; if not validName(r.username) then return {success=false,error="Invalid username"} end; local amount=math.floor(tonumber(r.amount) or 0); local a=account(r.username); if r.mode=="add" then a.balance=(a.balance or 0)+amount else a.balance=amount end; save(); return {success=true,username=r.username,balance=a.balance} end
 if r.action=="admin_catalog" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; local out={}; for i,v in pairs(catalog) do table.insert(out,{id=i,name=v.name or i,buy=v.buy or 0,sell=v.sell or 0}) end; table.sort(out,function(a,b) return a.id<b.id end); return {success=true,items=out} end
 if r.action=="admin_set_price" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; if not catalog[r.item] then catalog[r.item]={name=r.item} end; catalog[r.item].buy=math.max(0,math.floor(tonumber(r.buy) or 0)); catalog[r.item].sell=math.max(0,math.floor(tonumber(r.sell) or 0)); saveCatalog(); return {success=true} end
 if r.action=="admin_import" then if not adminSessions[sid] then return {success=false,error="Admin login required"} end; local imported=nil; if fs.exists("mcshop_import") then local f=fs.open("mcshop_import","r"); imported=textutils.unserialize(f.readAll()); f.close() elseif importURL~="" then local h,e=http.get(importURL); if h then imported=textutils.unserializeJSON(h.readAll()); h.close() else return {success=false,error="Import download failed: "..tostring(e)} end else return {success=false,error="mcshop_import is missing and shop_import_url is not configured"} end; if type(imported)~="table" then return {success=false,error="Invalid import file"} end; local count=0; for i,v in pairs(imported) do if validItem(i) and type(v)=="table" then catalog[i]={name=v.name or i,buy=tonumber(v.buy) or 0,sell=tonumber(v.sell) or 0}; count=count+1 end end; saveCatalog(); return {success=true,count=count} end
 local s=session(sid)
 ...
 if r.action=="loan_status" then return {success=true,loan=db.loans[s.username]} end
 if r.action=="loan_quote" then local amount=math.floor(tonumber(r.amount) or 0); if amount<1 or amount>MAX_LOAN then return {success=false,error="Loan must be between 1 and "..MAX_LOAN} end; return {success=true,quote=loanQuote(amount)} end
 if r.action=="loan_request" then local amount=math.floor(tonumber(r.amount) or 0); if amount<1 or amount>MAX_LOAN then return {success=false,error="Loan must be between 1 and "..MAX_LOAN} end; local old=db.loans[s.username]; if old and (old.status=="active" or old.status=="pending") then return {success=false,error="You already have an active or pending loan"} end; if old and (old.arrears or 0)>0 then return {success=false,error="Clear your arrears first"} end; local q=loanQuote(amount); db.loans[s.username]={status="pending",principal=amount,remaining=amount,initiationFee=q.initiationFee,netPayout=q.netPayout,totalInterest=q.totalInterest,totalRepayment=q.totalRepayment,termDays=LOAN_DAYS,dailyPrincipal=math.ceil(amount/LOAN_DAYS),interestRate=DAILY_INTEREST,paidDays=0,arrears=0,submittedDay=os.day(),installments=q.installments}; save(); return {success=true,loan=db.loans[s.username]} end
 if r.action=="history" then return {success=true,transactions=a.history or {}} end
 if r.action=="logout" then db.sessions[sid]=nil; return {success=true} end
