...
local function handle(id,r)
 ...
 if r.action=="admin_loan_decide" then if not adminSessions[id] then return {success=false,error="Admin login required"} end; local l=db.loans[r.username]; if not l or l.status~="pending" then return {success=false,error="No pending loan for that player"} end; if r.decision=="approve" then l.status="active"; l.lastPaidDay=os.day(); local a=account(r.username); a.balance=a.balance+l.netPayout; log(r.username,"loan_approved","loan",1,l.netPayout); save(); return {success=true,status="active"} elseif r.decision=="reject" then l.status="rejected"; save(); return {success=true,status="rejected"} end; return {success=false,error="Decision must be approve or reject"} end
