...
local function bank()
 while true do
  clear(); box(1,1,W,2,colors.gray,colors.yellow,"BANK"); term.setTextColor(colors.white); term.setCursorPos(2,4); term.write("Account: "..username); term.setCursorPos(2,5); term.write("Balance: "..balance); button(2,8,18,"TRANSFER",colors.blue); button(22,8,18,"TAKE LOAN",colors.orange); button(2,10,18,"LOAN STATUS",colors.purple); button(22,10,18,"BACK",colors.red); local e,b,x,y=os.pullEvent("mouse_click")
  ...
  elseif y==8 and x>=22 and x<40 then term.setCursorPos(2,13); term.write("Loan amount: "); local amount=tonumber(read()); local r=request({action="loan_request",amount=amount}); term.setCursorPos(2,15); term.write(fit(r.error or "Loan approved",W-3)); if r.success then balance=r.balance end; waitKey()
  elseif y==10 and x>=2 and x<20 then local r=request({action="loan_status"}); clear(); term.write("LOAN STATUS"); term.setCursorPos(2,4); if not r.loan then term.write("No loan") else local l=r.loan; term.write("Status: "..l.status); term.setCursorPos(2,5); term.write("Remaining: "..l.remaining); term.setCursorPos(2,6); term.write("Arrears: "..(l.arrears or 0)); term.setCursorPos(2,7); term.write("Paid days: "..l.paidDays) end; waitKey() end
 end
end
