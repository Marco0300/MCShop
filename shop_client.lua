...
local function bank()
 while true do
  clear(); box(1,1,W,2,colors.gray,colors.yellow,"BANK"); term.setTextColor(colors.white); term.setCursorPos(2,4); term.write("Account: "..username); term.setCursorPos(2,5); term.write("Balance: "..balance); button(2,8,18,"TRANSFER",colors.blue); button(22,8,18,"LOAN APPLICATION",colors.orange); button(2,10,18,"LOAN STATUS",colors.purple); button(22,10,18,"BACK",colors.red); local e,b,x,y=os.pullEvent("mouse_click")
  if y==10 and x>=22 and x<40 then return
  elseif y==8 and x>=2 and x<20 then term.setCursorPos(2,13); term.write("Recipient: "); local target=read(); term.setCursorPos(2,14); term.write("Amount: "); local amount=tonumber(read()); local r=request({action="transfer",target=target,amount=amount}); term.setCursorPos(2,16); term.write(fit(r.error or "Transfer complete",W-3)); if r.success then balance=r.balance end; waitKey()
  elseif y==8 and x>=22 and x<40 then term.setCursorPos(2,13); term.write("Loan amount: "); local amount=tonumber(read()); local q=request({action="loan_quote",amount=amount}); if not q.success then term.setCursorPos(2,15); term.write(fit(q.error,W-3)); waitKey() else local v=q.quote; term.setCursorPos(2,14); term.write(fit("Fee: "..v.initiationFee.."  Net payout: "..v.netPayout,W-3)); term.setCursorPos(2,15); term.write(fit("Interest: "..v.totalInterest.."  Total repay: "..v.totalRepayment,W-3)); term.setCursorPos(2,16); term.write("Click? Type Y to apply, N to cancel: "); local yes=read():lower(); if yes=="y" then local r=request({action="loan_request",amount=amount}); term.setCursorPos(2,18); term.write(fit(r.error or "Application submitted for approval",W-3)) end; waitKey() end
  elseif y==10 and x>=2 and x<20 then local r=request({action="loan_status"}); clear(); term.write("LOAN STATUS"); term.setCursorPos(2,4); if not r.loan then term.write("No loan application") else local l=r.loan; term.write("Status: "..l.status); term.setCursorPos(2,5); term.write("Principal: "..l.principal); term.setCursorPos(2,6); term.write("Remaining: "..(l.remaining or 0)); term.setCursorPos(2,7); term.write("Arrears: "..(l.arrears or 0)); term.setCursorPos(2,8); term.write("Total repay: "..(l.totalRepayment or 0)) end; waitKey() end
 end
end
...
local function draw()
 ...
 local tx=1; for _,c in ipairs(categories) do local cw=#c+2; if tx+cw<=W-9 then button(tx,3,cw,c,c==category and colors.orange or colors.blue); tx=tx+cw+1 end end; button(W-8,3,8,"LOGIN",colors.green); button(W-8,4,8,"HISTORY",colors.purple); button(W-8,5,8,"BANK",colors.blue)
...
while true do
 ...
 elseif e=="mouse_click" then
  ...
  elseif y==5 and x>=W-8 then if logged then bank() else login() end
